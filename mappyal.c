...
int MapLoadMAR (char * mname, int marlyr)
{
    ...
    if (pack_fread (mapmappt[marlyr], (mapwidth*mapheight*sizeof(short int)), marfpt) !=
            (mapwidth*mapheight*sizeof(short int)))
    {
        pack_fclose (marfpt);
        return -1;
    }
...
void MapSetPal8 (void)
{
    if (screen!=NULL)
    {
        if ((bitmap_color_depth (screen)==8) && mapdepth == 8) set_palette (mapcmap6bit);
    }
}
...
//long int * myanblkpt;
    ...
    {
        j = 0;
        while (mapnovctext[i] >= '0' && mapnovctext[i] <= '9')
        {
...
//BLKSTR * myblkstrpt;
    ...
    for (i=0; i<(mapblockwidth*mapblockheight*mapnumblockgfx); i++)
    {
        ...
        {
        case 8:
            if (cdepth==8) pixcol = (int) *oldgfxpt;
            else
            {
                j = (*oldgfxpt)*3;
        ...
        case 32:
            ...
            if (cdepth==8)
            {
                if (*(oldgfxpt+1) == 0xFF && *(oldgfxpt+2) == 0 &&
...
int MapDecodeBKDT (unsigned char * mdatpt)
{
    ...
    while (i < (mapnumblockstr*mapblockstrsize))
    {
        ...
        if (mdatpt[i+31]&0x80) myblkpt->unused3 = 1;
        else myblkpt->unused3 = 0;
        if (mdatpt[i+31]&0x40) myblkpt->unused2 = 1;
        else myblkpt->unused2 = 0;
        if (mdatpt[i+31]&0x20) myblkpt->unused1 = 1;
        else myblkpt->unused1 = 0;
        if (mdatpt[i+31]&0x10) myblkpt->trigger = 1;
        else myblkpt->trigger = 0;
        if (mdatpt[i+31]&0x08) myblkpt->br = 1;
        else myblkpt->br = 0;
        if (mdatpt[i+31]&0x04) myblkpt->bl = 1;
        else myblkpt->bl = 0;
        if (mdatpt[i+31]&0x02) myblkpt->tr = 1;
        else myblkpt->tr = 0;
        if (mdatpt[i+31]&0x01) myblkpt->tl = 1;
        else myblkpt->tl = 0;
        i += mapblockstrsize;
        myblkpt++;
    }
                    ...
                    for (j=0; j<mapheight; j++)
                    {
                        for (i=0; i<mapwidth;)
                        {
                            ...
                            else
                            {
                                if (k < 0)
                                {
                                    mymap2pt = mymappt + (int) MapGetshort (mdatpt);
                                    mdatpt += 2;
                                    while (k)
                                    {
                                        *mymappt = *mymap2pt;
...
int MapRealDecode (PACKFILE * mfpt, unsigned char * mmpt, long int mpfilesize)
{
    ...
    while (mpfilesize > 0)
    {
        ...
        */
        if (!strncmp (fmappospt, "CMAP", 4))
        {
            chkdn = 1;
            MapDecodeCMAP (fmappospt);
        }
        if (!strncmp (fmappospt, "BKDT", 4))
        {
            chkdn = 1;
            MapDecodeBKDT (fmappospt);
        }
        if (!strncmp (fmappospt, "ANDT", 4))
        {
            chkdn = 1;
            MapDecodeANDT (fmappospt);
        }
        if (!strncmp (fmappospt, "AGFX", 4))
        {
            chkdn = 1;
            MapDecodeAGFX (fmappospt);
        }
        if (!strncmp (fmappospt, "BGFX", 4))
        {
            chkdn = 1;
            MapDecodeBGFX (fmappospt);
        }
        if (!strncmp (fmappospt, "NOVC", 4))
        {
            chkdn = 1;
            MapDecodeNOVC (fmappospt);
        }
        if (!strncmp (fmappospt, "BODY", 4))
        {
            chkdn = 1;
            MapDecodeLayer (fmappospt, 0);
        }
        if (!strncmp (fmappospt, "LYR1", 4))
        {
            chkdn = 1;
            MapDecodeLayer (fmappospt, 1);
        }
        if (!strncmp (fmappospt, "LYR2", 4))
        {
            chkdn = 1;
            MapDecodeLayer (fmappospt, 2);
        }
        if (!strncmp (fmappospt, "LYR3", 4))
        {
            chkdn = 1;
            MapDecodeLayer (fmappospt, 3);
        }
        if (!strncmp (fmappospt, "LYR4", 4))
        {
            chkdn = 1;
            MapDecodeLayer (fmappospt, 4);
        }
        if (!strncmp (fmappospt, "LYR5", 4))
        {
            chkdn = 1;
            MapDecodeLayer (fmappospt, 5);
        }
        if (!strncmp (fmappospt, "LYR6", 4))
        {
            chkdn = 1;
            MapDecodeLayer (fmappospt, 6);
        }
        if (!strncmp (fmappospt, "LYR7", 4))
        {
            chkdn = 1;
        ...
        for (i+=((mapx-maphclip)-mbgx); i<((mapx+mapw)); i+=mapblockgapx)
        {
            ...
            do
            {
                ...
                for (; bfield<4; bfield++)
                {
                    ...
                    {
                    case 0:
                        mapgfxpt = (BITMAP *) blkdatapt->bgoff;
                        break;
                    case 1:
                        mapgfxpt = (BITMAP *) blkdatapt->fgoff;
                        break;
                    case 2:
                        mapgfxpt = (BITMAP *) blkdatapt->fgoff2;
                        break;
                    default:
                    case 3:
                        mapgfxpt = (BITMAP *) blkdatapt->fgoff3;
                        break;
                    }
                    if (((int)mapgfxpt) != 0)
                    {
